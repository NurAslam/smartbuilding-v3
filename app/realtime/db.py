...
def insert_row(row: dict):
    ...
    INSERT INTO sensors_hourly
        (ts, temp, humidity, wind_speed, pm25, energy_kwh, cost_idr, eui_kwh_m2,
         pmv, ppd, pmv_label, dayofweek)
    VALUES
        (%(ts)s, %(temp)s, %(humidity)s, %(wind_speed)s, %(pm25)s, %(energy_kwh)s, %(cost_idr)s, %(eui_kwh_m2)s,
         %(pmv)s, %(ppd)s, %(pmv_label)s, %(dayofweek)s)
